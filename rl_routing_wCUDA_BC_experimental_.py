...
This experimental variant adds behavior cloning (BC) support:
1) Generate expert demonstrations via shortest-path routing.
2) Optionally prefill replay with expert transitions.
3) Optionally run a BC pretraining phase before RL.
4) Optionally keep an auxiliary BC loss during RL fine-tuning.
"""
...
DEFAULT_CONFIG = {
    ...
    "behavior_cloning": {
        "enabled": False,
        "expert_weight": "travel_time",  # travel_time | base_time | length
        "demo_episodes": 200,
        "max_demo_steps_per_episode": 0,  # 0 -> use env.max_steps
        "prefill_replay": True,
        "prefill_limit": 20000,  # cap expert transitions copied into replay
        "pretrain_steps": 0,  # set >0 to run BC pretraining before RL
        "batch_size": 128,
        "lr": 3e-4,
        "aux_loss_weight_start": 0.20,  # BC loss multiplier during RL updates
        "aux_loss_decay": 0.9995,
        "aux_loss_weight_min": 0.0,
    },
    "evaluation": {
        "episodes": 300,
        "epsilon_greedy": 0.0,
...
def _sample_with_replacement(data, batch_size):
    if len(data) >= batch_size:
        return random.sample(data, batch_size)
    return random.choices(data, k=batch_size)


def _expert_target_node(env, unvisited, expert_weight):
    if not unvisited:
        return None
    try:
        dist_map = nx.single_source_dijkstra_path_length(env.G, env.current_node, weight=expert_weight)
    except Exception:
        dist_map = {}
    return min(unvisited, key=lambda d: dist_map.get(d, float("inf")))


def select_expert_action(env, expert_weight="travel_time"):
    """
    Expert policy used for BC data generation.
    Prefers the shortest-path next hop toward the nearest unvisited delivery.
    Falls back to the valid neighbor that minimizes downstream distance.
    """
    ...
    unvisited = [d for d in env.delivery_nodes if d not in env.completed]
    if not unvisited:
        return int(valid_actions[0]), mask

    target = _expert_target_node(env, unvisited, expert_weight=expert_weight)
    next_hop = None
    ...
    action_slots = list(env._action_slots)
    if next_hop in action_slots:
        slot_idx = action_slots.index(next_hop)
        if slot_idx in valid_actions:
            return int(slot_idx), mask

    # Fallback: choose valid neighbor that is closest (by expert_weight) to any unvisited delivery.
    best_action = int(valid_actions[0])
    best_score = float("inf")
    for action_idx in valid_actions:
        nbr = action_slots[action_idx]
        try:
            nbr_dist_map = nx.single_source_dijkstra_path_length(env.G, nbr, weight=expert_weight)
            score = min(nbr_dist_map.get(d, float("inf")) for d in unvisited)
        except Exception:
            score = float("inf")
        if score < best_score:
            best_score = score
...
def generate_expert_demonstrations(
    ...
    expert_weight="travel_time",
):
    """
    Roll out the expert policy in the environment to build:
    - replay transitions (for warm-start replay prefill)
    - BC samples (state + mask + expert action)
    """
    replay_transitions = []
    bc_samples = []
    successes = 0
    step_limit = int(max_steps_per_episode) if int(max_steps_per_episode) > 0 else int(env.max_steps)
    ...
    for _ in range(int(num_episodes)):
        ...
        while (not done) and (steps < step_limit):
            expert_action, mask = select_expert_action(env, expert_weight=expert_weight)
            ...
            replay_transitions.append(
                (
                    ...
                    state["unvisited_idx"].clone(),
                    state["unvisited_mask"].clone(),
                    int(expert_action),
                    float(reward),
                    next_state["state_vec"].clone(),
                    next_state["current_idx"].clone(),
                    next_state["unvisited_idx"].clone(),
                    next_state["unvisited_mask"].clone(),
                    bool(done),
                    next_mask.clone(),
                )
            )
    ...
    success_rate = successes / max(int(num_episodes), 1)
    return replay_transitions, bc_samples, success_rate


def sample_bc_batch(bc_samples, batch_size):
    if (not bc_samples) or batch_size <= 0:
        return None
    batch = _sample_with_replacement(bc_samples, int(batch_size))
    state_vecs, current_idxs, unvisited_idxs, unvisited_masks, actions, masks = zip(*batch)
    return (
        torch.stack(state_vecs),
        torch.stack(current_idxs).to(dtype=torch.long).view(-1),
        torch.stack(unvisited_idxs),
        torch.stack(unvisited_masks),
        torch.tensor(actions, dtype=torch.long),
        torch.stack(masks),
...
def pretrain_with_behavior_cloning(
    ...
    if (not bc_samples) or int(pretrain_steps) <= 0:
        return None

    criterion = nn.CrossEntropyLoss()
    model.train()
    total_steps = int(pretrain_steps)
    interval = max(1, total_steps // 10)
    last_loss = None

    for step in range(total_steps):
        batch = sample_bc_batch(bc_samples, int(batch_size))
        if batch is None:
            return last_loss
        state_vecs, current_idxs, unvisited_idxs, unvisited_masks, actions, masks = batch
        ...
        logits = model(state_vecs, current_idxs, unvisited_idxs, unvisited_masks)
        ...
        if log_fn and ((step + 1) % interval == 0 or step == 0):
            log_fn(f"[BC Pretrain] Step {step + 1}/{total_steps}, Loss: {last_loss:.6f}")
...
def train(config_path=CONFIG_PATH_DEFAULT, config_overrides=None):
    ...
    try:
        ...
        bc_expert_weight = str(bc_cfg.get("expert_weight", "travel_time"))
        bc_demo_episodes = max(0, int(bc_cfg.get("demo_episodes", 0)))
        bc_demo_max_steps = max(0, int(bc_cfg.get("max_demo_steps_per_episode", 0)))
        bc_prefill_replay = bool(bc_cfg.get("prefill_replay", True))
        bc_prefill_limit = max(0, int(bc_cfg.get("prefill_limit", 0)))
        bc_pretrain_steps = max(0, int(bc_cfg.get("pretrain_steps", 0)))
        bc_batch_size = max(1, int(bc_cfg.get("batch_size", batch_size)))
        bc_pretrain_lr = float(bc_cfg.get("lr", train_cfg["lr"]))
        bc_aux_weight = float(bc_cfg.get("aux_loss_weight_start", 0.0))
        bc_aux_decay = float(bc_cfg.get("aux_loss_decay", 1.0))
        bc_aux_min = float(bc_cfg.get("aux_loss_weight_min", 0.0))
        bc_samples = []
        ...
        if bc_enabled:
            log(
                "BC enabled | "
                f"expert_weight={bc_expert_weight}, demo_episodes={bc_demo_episodes}, "
                f"prefill_replay={bc_prefill_replay}, pretrain_steps={bc_pretrain_steps}, "
                f"aux_w_start={bc_aux_weight:.4f}, aux_w_decay={bc_aux_decay:.6f}, aux_w_min={bc_aux_min:.4f}"
            )

            replay_demos, bc_samples, demo_success_rate = generate_expert_demonstrations(
                env,
                num_episodes=bc_demo_episodes,
                max_steps_per_episode=bc_demo_max_steps,
                expert_weight=bc_expert_weight,
            )
            log(
                f"Generated expert demos | transitions={len(replay_demos)}, "
                f"bc_samples={len(bc_samples)}, expert_success={demo_success_rate:.2%}"
            )

            if bc_prefill_replay and replay_demos:
                prefill_count = len(replay_demos)
                if bc_prefill_limit > 0:
                    prefill_count = min(prefill_count, bc_prefill_limit)
                for transition in replay_demos[:prefill_count]:
                    buffer.store(transition)
                log(f"Prefilled replay with {prefill_count} expert transitions.")

            if bc_pretrain_steps > 0 and bc_samples:
                original_lrs = [pg["lr"] for pg in optimizer.param_groups]
                for pg in optimizer.param_groups:
                    pg["lr"] = bc_pretrain_lr
                last_bc_loss = pretrain_with_behavior_cloning(
                    online,
                    optimizer,
                    bc_samples=bc_samples,
                    device=device,
                    pretrain_steps=bc_pretrain_steps,
                    batch_size=bc_batch_size,
                    log_fn=log,
                )
                for pg, old_lr in zip(optimizer.param_groups, original_lrs):
                    pg["lr"] = old_lr
                target.load_state_dict(online.state_dict())
                if last_bc_loss is not None:
                    log(f"BC pretraining finished | final_loss={last_bc_loss:.6f}")
        ...
        for episode in range(num_episodes):
            ...
            while not done:
                ...
                if len(buffer) >= batch_size:
                    ...
                    td_loss = td_criterion(q_selected, target_q)
                    loss = td_loss
                    bc_loss = None

                    if bc_enabled and bc_samples and bc_aux_weight > 0.0:
                        bc_batch = sample_bc_batch(bc_samples, bc_batch_size)
                        if bc_batch is not None:
                            ...
                            bc_state_vecs = bc_state_vecs.to(device)
                            bc_current_idxs = bc_current_idxs.to(device)
                            bc_unvisited_idxs = bc_unvisited_idxs.to(device)
                            bc_unvisited_masks = bc_unvisited_masks.to(device)
                            bc_actions = bc_actions.to(device)
                            bc_masks = bc_masks.to(device)
                            ...
                            bc_logits = bc_logits.masked_fill(bc_masks <= 0, -1e9)
                            bc_loss = bc_criterion(bc_logits, bc_actions)
                            loss = td_loss + (bc_aux_weight * bc_loss)

                    optimizer.zero_grad()
                    loss.backward()
                    optimizer.step()
                    if bc_loss is not None:
                        bc_aux_weight = max(bc_aux_min, bc_aux_weight * bc_aux_decay)
            ...
            if (episode + 1) % log_every == 0:
                ...
                log(
                    ...
                    + (f", BCw: {bc_aux_weight:.4f}" if bc_enabled else "")
                )
                    ...
                    torch.save(
                        {
                            ...
                            "behavior_cloning_enabled": bc_enabled,
                            "behavior_cloning_aux_weight": bc_aux_weight,
...
# python rl_routing_wCUDA_BC_experimental_.py --config configs/no_hazard_config_BC.json
